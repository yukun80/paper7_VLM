...
def _source_record_fingerprint(bank: QwenVisionFeatureBank, key: str) -> str:
    return str(bank.task_neutral_record(key)["cache_fingerprint"])


def _deep_validation_report(
    output: Path,
    *,
    ...
    report = bank.validate_all(
        ...
        source_record_fingerprint=(
            (lambda key: _source_record_fingerprint(source_bank, key))
...
def main() -> None:
    ...
    if args.verify_only:
        ...
        report = _deep_validation_report(
            output,
    ...
    if not source_unchanged:
        ...
        _write_json_atomic(output / "validation_report.json", invalid_report)
    ...
    validation_report = _deep_validation_report(
        output,
        input_fingerprints=published_input_fingerprints,
        source_bank=segmentation_bank,
        source_cache_path=source_cache_path,
    )
    _write_json_atomic(output / "validation_report.json", validation_report)
