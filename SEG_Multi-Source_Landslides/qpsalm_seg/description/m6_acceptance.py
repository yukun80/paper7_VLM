...
from .evaluator import (
    ...
    revalidate_evaluation_mask_artifact,
)
...
def _evaluation_artifact(
    *,
    ...
    rows, report = _rows(root, require_complete_generation=True)
    if (
    ...
    checkpoint_payload_provenance = _validate_evaluation_checkpoint_provenance(
        root, report
    )
    ...
    checkpoint_description_benchmark = dict(
        metadata.get("config") or {}
    ).get("description_benchmark")
    ...
    return {
        ...
        "passed": bool(
            absolute_gate.get("passed") and counterfactual_gate.get("passed")
        ),
    }
