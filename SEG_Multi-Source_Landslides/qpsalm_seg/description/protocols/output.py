...
def canonical_description_json(value: dict[str, Any]) -> str:
    """Serialize one already-valid output in the causal schema field order.

    This function is deliberately strict: it neither repairs values nor fills
    missing fields.  Training targets and constrained inference both call this
    serializer so the decoder sees the same key order in both directions.
    """

    if not isinstance(value, dict):
        raise ValueError("description output 必须是 JSON object")
    errors = _validation_errors(value)
    if errors:
        raise ValueError(
            "description output 不符合 qpsalm_description_output_v1: "
            f"{errors}"
        )
    region = value["region"]
    ...
    ordered = {
        ...
        "evidence": {
            "surface_observation": evidence["surface_observation"],
            "terrain_support": evidence["terrain_support"],
            "sar_support": evidence["sar_support"],
            "deformation_support": evidence["deformation_support"],
            "surrounding_context": evidence["surrounding_context"],
            "evidence_sufficiency": evidence["evidence_sufficiency"],
        },
        "summary": value["summary"],
    }
    if "confidence" in value:
        ordered["confidence"] = value["confidence"]
    return json.dumps(
        ordered,
        ensure_ascii=False,
        separators=(",", ":"),
