...
from .backbone import restore_region_mask_from_cache
...
class DescriptionDemoSession:
    def __init__(
        ...
        self.end_to_end = (
            EndToEndMaskProvider(
                self.model, split, config.segmentation_mask_threshold
            )
    ...
    def _protocol_mask(
        ...
        return base, {
            "mask_source": (
                "fixed_prediction" if self.config.evaluation_mode == "fixed_prediction"
                else "gt_mask"
            ),
    ...
    def inspect(self, sample_id: str):
        ...
        return (
            ...
            item["instruction"],
            {
                ...
                "region_protocol": self.config.region_protocol,
