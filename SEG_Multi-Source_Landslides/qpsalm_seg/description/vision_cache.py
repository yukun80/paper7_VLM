...
# 正式 M4/M6 一次会重放同一 cache 的多个 checkpoint。只有 cache 目录内全部
# 文件元数据都未变化时才复用全 shard hash，普通写入/替换会强制重新扫描。
...
class DescriptionVisionFeatureBank:
    ...
    def artifact_binding(self) -> dict[str, Any]:
        ...
        return {
            ...
            "input_fingerprints_sha256": _canonical_sha256(
                self.manifest["input_fingerprints"]
            ),
            "source_cache_provenance_sha256": _canonical_sha256(
    ...
    def _file_metadata_snapshot(self) -> dict[str, Any]:
        ...
        return {
            "file_count": len(entries),
            "metadata_sha256": _canonical_sha256(entries),
        }
    ...
    def _validate_record(self, row: dict[str, Any]) -> None:
        ...
        for view in views:
            if not isinstance(view, dict):
                raise ValueError(f"description cache view 必须是 object: {row['lookup_key']}")
        ...
        if row.get("source_cache") and row.get("component") != "multisource_parent":
            raise ValueError(f"single_image record 禁止复用 segmentation cache: {row['lookup_key']}")
...
def revalidate_description_cache_artifact(
    ...
    binding_sha256 = _canonical_sha256(observed_binding)
    verification_key = _canonical_sha256({
