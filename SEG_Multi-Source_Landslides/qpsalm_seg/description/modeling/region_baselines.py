#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Controlled region-modeling baselines for MGRR ablations."""
        ...
        for batch_index, pyramids in enumerate(backbone.features.samples):
            ...
            for region_index in range(region_count):
                ...
                if is_present and protocol == "assisted":
                    geometry_input = geometry_values(mask, valid[batch_index])
                    geometry = self.geometry_project(geometry_input)
                elif self.mode == "full_image_box":
                    geometry_input = (
                        box_coordinate_values(mask, valid[batch_index])
                ...
                for pyramid in pyramids:
                    ...
                    if self.mode == "crop_only":
                        pooled = bbox_pool(feature, effective, modality_valid)
                    elif self.mode == "masked_pooling":
                        pooled = masked_pool(feature, effective)
                    else:
                        pooled = masked_pool(
                            feature,
                            modality_valid,
