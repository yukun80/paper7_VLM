#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""MGRR modeling over task-neutral SANE features."""
...
from ..protocols.region_geometry import region_mask_for_modality_view
...
def masked_pool(feature: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
...
def geometry_values(mask: torch.Tensor, valid: torch.Tensor) -> torch.Tensor:
...
def box_coordinate_values(mask: torch.Tensor, valid: torch.Tensor) -> torch.Tensor:
    """Keep only geometry that Vision-only explicitly receives as box input."""
    values = geometry_values(mask, valid)
...
class MultiGranularityRegionReplay(nn.Module):
    ...
    def _component_replay(
        ...
        for component in components:
            ...
            exact = masked_pool(aligned_feature, effective)
            roi = self._multi_scale_roi_replay(pyramid, value)
            component_ring = _context_ring(value, valid_mask)
            local_context = masked_pool(aligned_feature, component_ring * valid_mask)
        ...
        if residual is not None and self.ablation != "roi_replay_only":
            value = residual[None]
            exact = masked_pool(aligned_feature, value * valid_mask)
        ...
        *,
        ...
        for batch_index, pyramids in enumerate(backbone.features.samples):
            ...
            for region_index in range(region_count):
                region = (
                    (masks[
                        batch_index, region_index:region_index + 1
                    ] > 0.5)
                    & (valid[batch_index] > 0.5)
                ).float()
                ...
                geometry_input = geometry_values(region, valid[batch_index])
                ...
                for pyramid in pyramids:
                    ...
                    exact_detail = masked_pool(detail, effective_region)
                    exact_high = masked_pool(high, modality_region * high_valid)
                    context_raw = masked_pool(detail, effective_ring)
                    ...
                    global_token = self.global_project(masked_pool(
