...
AUDIT_BUILDER_VERSION = "sami_source_audit_v3_provenance_bound"
...
def _build_registry(config: BenchmarkAuditConfig) -> dict[str, Any]:
    """Project technical roles and minimal scientific provenance into the registry."""
...
def _build_provenance_report(registry: dict[str, Any]) -> dict[str, Any]:
    """Bind every configured source/component to its exact provenance row."""

    entries = registry["entries"]
    bindings = {
        entry["source_key"]: sha256_bytes(
            canonical_json_bytes(
    ...
    bindings.update(
        {
            component["component_key"]: sha256_bytes(canonical_json_bytes(component["provenance"]))
            for entry in entries
    ...
    return {
        ...
        "binding_sha256": dict(sorted(bindings.items())),
        "errors": [],
        "warnings": [],
    }
...
def audit_sources(
    ...
    *,
    ...
    manifest: dict[str, Any] = {
        ...
        "errors": [],
        "warnings": inventory["warnings"] + provenance_report["warnings"],
    }
    ...
    with atomic_output_directory(output_dir) as staging:
        ...
        atomic_write_bytes(staging / "provenance_report.json", provenance_bytes)
