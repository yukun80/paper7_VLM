...
结果原子写入
`outputs/qpsalm_description/cache/small_vision_v1/validation_report.json`。正式 M3 验收要求
...
改索引不能被后续课程静默接受。旧的 M3 v2
cache 没有 shard 内容哈希，不兼容且必须重建；Description cache 的公开 v1 format/protocol 和
segmentation cache v3 均未改变。之后可只读复核已有 cache，不重新编码视觉特征：
...
但 D0 必须额外提供当前 D-1 v7 统一门禁；该 acceptance 会沿后续 checkpoint lineage 传播并在
...
python -m qpsalm_seg.cli.eval_description_zero_shot \
  ...
  --split dev --seed 42 --device cuda --max-samples 64 \
...
python -m qpsalm_seg.cli.train_description \
  ...
  --stage overfit --seed 42 --region-encoder mgrr --device cuda --batch-size 2 \
  --max-steps 100 --max-train-samples 64 --max-val-samples 64 \
  --val-interval 25 --save-interval 50 \
  --output-dir outputs/qpsalm_description/d_minus_1_overfit_m5v5_seed42 \
  --overwrite-output
```
...
summary、gradient gate、trainable manifest、validation/raw generation、checkpoint 和显式
segmentation migration，并独立绑定 ontology、record schema 与 output schema；后续 gate 会按
当前仓库字节级重验三份协议资产。当前 overfit report 为
`qpsalm_d_minus_one_overfit_validation_v5_strict_json_finite`，还会从 checkpoint 本体重放
format/step/metadata、segmentation migration 和精确 M3 cache 的全部 shard。reload 会先破坏一个
`desc_adapter` LoRA 哨兵，再通过严格 model/
optimizer/scheduler loader 恢复；24 GiB 门禁要求真实 CUDA 记录和正的峰值，CPU 产生的
`0 GiB` 不再算通过。旧 v1-v4 overfit report 不兼容，需重跑该短 run。该报告不会单独声称
D-1 完成。
...
```bash
PYTHONPATH=SEG_Multi-Source_Landslides \
python -m qpsalm_seg.cli.validate_d_minus_one \
  --zero-shot-dir outputs/qpsalm_description/d_minus_1_zero_shot_dev \
  --overfit-dir outputs/qpsalm_description/d_minus_1_overfit_m5v5_seed42 \
  --output outputs/qpsalm_description/d_minus_1_gate_seed42.json
```

统一门禁使用 `qpsalm_d_minus_one_engineering_gate_v7_training_completion_bound`，会从当前 M1.1
索引重新选择 zero-shot population，并逐张重开所选 M1.1 `data/` 图像，要求路径属于当前
benchmark、`storage_mode=materialized_copy` 且 live SHA 与 `visual_ref.sha256` 一致；同时重验模型
元数据、checkpoint payload、Description cache 及上述 overfit 源文件。它还要求 overfit run 已原子
...
python -m qpsalm_seg.cli.train_description \
  ...
  --d-minus-one-gate outputs/qpsalm_description/d_minus_1_gate_seed42.json \
...
`qpsalm_d_minus_one_acceptance_v5_training_completion_bound`，把 M1.1 benchmark root、builder、
...
`qpsalm_description_evaluation_v16_atomic_artifact_bound` 的完整 GT-mask、Vision-only
population
