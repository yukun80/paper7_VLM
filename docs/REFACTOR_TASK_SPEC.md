...
4. 原始 source registry、数据许可证、来源 hash；
...
正式 training-eligible index 不接受 `license_status=unknown`。未知来源可以留在 audit inventory，但不能静默进入训练。
...
| RSGPT | RSICap/RSIEval 数据职责 | 高质量人工 caption 与独立 test | 数据按授权使用 | 代码根 LICENSE 未发现，禁止复制；图像 academic-only | DOTA/RSICap 商用受限 |
| EarthGPT | MMRS 子集和任务格式 | source-specific data audit | 仅解析已获授权的数据 | 根 LICENSE 未发现；不得复制代码或整库数据 | 各组成数据许可分别审核 |
| Qwen3-VL-Seg | box-guided 17M decoder | GT-box/predicted-box 分层评价 | 独立按论文实现 | 未确认官方代码；第三方复现不视为官方 | P0 记录论文和独立实现来源 |

Apache-2.0 与 MIT 代码通常可在保留通知条件下组合，但当前项目公开根目录未发现 LICENSE。P0 必须由人工选择新项目许可证；在此之前不得发布重构代码或权重。
...
| Phase | 目标 | 关键输出 | 阶段验收 |
|---|---|---|---|
| P0 | 实时审计与设计冻结 | inventory、license、ADR、backup plan | 人工批准 ADR 和备份 |
...
4. 检查本地 `external/`；
5. 生成 reuse/deletion/license matrix；
6. 写 ADR-0001；
7. 提出 backup 命令。
...
- 不读取旧派生 Benchmark；
- license report 无 training-eligible unknown。
...
11. license/NOTICE 和 data usage 记录完整；
12. 未把工程门禁写成科学成功。
...
| 需要人工决定 | baseline tag/branch；项目根许可证；数据使用许可；G0 formal 运行；付费 API；专家审核/仲裁；正式训练；最终阈值 |
