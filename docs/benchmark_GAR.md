...
5. `target_status=absent` 时明确拒绝，不继续生成滑坡属性；
...
holdout hash。`qpsalm-merge-oof-predictions` 只有在每个 parent 恰好出现一次、fold 归属正确且
`out_of_fold_verified=true` 时才发布 D4 train index。仅传入一个 fold 名称而不满足这些指纹
约束不算 out-of-fold。
